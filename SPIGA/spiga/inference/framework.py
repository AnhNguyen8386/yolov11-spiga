...
class SPIGAFramework:
    ...
    def __init__(self, model_cfg: ModelConfig(), gpus=[0], load3DM=True):
        ...
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")  # ✅ Device logic
        ...
        self.model = self.model.to(self.device)  # ✅ Sửa lại để dùng CPU nếu không có GPU
        self.model.eval()
        print(f'SPIGA model loaded on {self.device}!')
